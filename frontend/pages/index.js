    ...
    <MainLayout title='Beach Resort ― Home'>
      <Hero>
        <Banner
          ...
          subtitle='deluxe rooms starting at ₹899'
        >
...
export async function getServerSideProps() {
  try {
    ...
    const response = await axios.get(`${API_BASE_URL}/api/v1/featured-rooms-list`);
    ...
    // ensure error is serializable (plain object or null)
    const errorPayload = err?.response?.data || { message: err?.message || 'Server error' };
    return {
      props: {
        featuredRooms: null,
        error: errorPayload
