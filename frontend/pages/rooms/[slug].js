...
function RoomPreview(props) {
  ...
  const getImageUrl = (url) => {
    ...
    // If the URL contains localhost:8080 followed by https, extract the https part
    if (url.includes('localhost:8080https')) {
...
export async function getServerSideProps(ctx) {
  try {
    ...
    const response = await axios.get(
      `${API_BASE_URL}/api/v1/get-room-by-id-or-slug-name/${ctx.query.slug}`
    );
    ...
    const errorPayload = {
      message: err?.response?.data?.message || err?.message || 'Failed to fetch room details',
