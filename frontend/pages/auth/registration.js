...
function Registration() {
  ...
  const onFinish = async (values) => {
    ...
    try {
      const response = await ApiService.post('/api/v1/auth/registration', data);
      // normalize response (ApiService may return response.data or the full response)
      const resp = response?.data ?? response;

      if (resp?.result_code === 0) {
        notificationWithIcon('success', 'SUCCESS', resp?.result?.message || 'Your registration successful');
        form.resetFields();
        router.push('/auth/login');
      } else {
        const msg = resp?.result?.message || resp?.message || 'Sorry! Something went wrong. App server error';
        notificationWithIcon('error', 'ERROR', msg);
      }
    } catch (err) {
      const msg = err?.response?.data?.result?.error?.message
