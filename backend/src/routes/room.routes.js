...
// route for create new room (supports both formdata and JSON)
router.route('/create-room').post(isAuthenticatedUser, isAdmin, (req, res, next) => {
  // Skip multer if Content-Type is not multipart/form-data
  if (req.headers['content-type'] && req.headers['content-type'].includes('multipart/form-data')) {
    req.isFormData = true;
    roomImageUpload.array('room_images', 5)(req, res, next);
  } else {
